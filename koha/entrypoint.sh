...
END_COMMENT
    ...
    LINE=$(grep -n '<pass>' /etc/koha/sites/koha/koha-conf.xml | cut -d ':' -f1)
    sed -i "$LINE d" /etc/koha/sites/koha/koha-conf.xml
    sed -i "$LINE i\ <pass>$MASTER_PASSWORD</pass>" /etc/koha/sites/koha/koha-conf.xml
